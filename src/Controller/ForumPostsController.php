...
final class ForumPostsController extends AbstractController{
   ...
   $search = $request->query->get('search');
    ...
    public function like(ForumPosts $forumPost, EntityManagerInterface $entityManager, Request $request): Response
    {
        if ($request->isXmlHttpRequest()) { // Vérifie si la requête est AJAX
            ...
            // Retourne une réponse JSON avec le nouveau nombre de likes
            return $this->json([
    ...
    public function dislike(ForumPosts $forumPost, EntityManagerInterface $entityManager, Request $request): Response
    {
        if ($request->isXmlHttpRequest()) { // Vérifie si la requête est AJAX
            ...
            // Retourne une réponse JSON avec le nouveau nombre de dislikes
            return $this->json([
                'dislikes' => $forumPost->getDislikes(),
            ]);
        }
        return $this->redirectToRoute('app_forum_posts_index', ['id' => $forumPost->getId()]);
    }
